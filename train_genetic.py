...
from network.workers import custom_worker_conv_against_monte_carlo
from utils.consts import NO_WALLS, SAVED_NETWORKS_DIR
from utils.functions import Functions

if __name__ == '__main__':
    MasterNode = ActorCriticConv()
    MasterNode.load_state_dict(torch.load(SAVED_NETWORKS_DIR + "test.pt"))
    ...
    for i in range(it):
        p = mp.Process(target=custom_worker_conv_against_monte_carlo, args=(i, MasterNode, ep_list, loss_list, params))
        p.start()
        processes.append(p)
